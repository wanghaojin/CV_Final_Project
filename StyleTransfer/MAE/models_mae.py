...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio, mask=None):
        ...
        # else:
        #     # 使用提供的mask和ids_restore
        #     x = torch.gather(x, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # [N, L, D]
        #     x = x * (1 - mask.unsqueeze(-1))  # Apply mask
